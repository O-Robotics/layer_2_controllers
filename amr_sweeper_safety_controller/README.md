...
## Overview
`amr_sweeper_safety_controller` subscribes to one shared typed stop topic that any package can publish to. On the first stop request it latches the event, republishes zero wheel commands into a dedicated `twist_mux` safety-stop input, republishes zero tool commands, and keeps the robot stopped until a manual reset service is called.
