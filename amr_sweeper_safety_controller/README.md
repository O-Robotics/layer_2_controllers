...
## Overview
`amr_sweeper_safety_controller` subscribes to one shared typed stop topic that any package can publish to. On the first stop request it latches the event, republishes zero wheel commands into the sweeping-controller safety-stop input used at highest priority for both wheels and tools, and sends direct CAN stop frames to the Layer 1 ODrive and Steadydrive motor nodes. While the stop is latched it also verifies from motor feedback that the drives actually came to rest. The stop stays latched until a manual reset service is called.
