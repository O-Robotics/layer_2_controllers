...
## Overview
Layer 2 sits between the hardware interfaces in layer 1 and the higher-level decision-making in layer 3. It contains the joystick input path, the command arbitration path for wheel motion, the relay into the wheel ODrive controller, the tool command mapper for the SteadyDrive tool motors, and the attitude supervision path driven by the robot IMU.
...
- The attitude controller consumes `imu/data_raw`, which resolves to `/amr_sweeper/imu/data_raw` under the default namespace, and publishes namespaced attitude and safety topics by default.
